...
start cmd /k "cd ../kernel && go run kernel.go configs/config_PRUEBA_IO.json"
...
start cmd /k "cd ../memoria && go run memoria.go configs/config_PRUEBA_IO.json"

echo Waiting for memoria.go to load (10 seconds)...
timeout /t 10 /nobreak >nul

echo Running cpu.go in a new console window...
start cmd /k "cd ../cpu && go run cpu.go configs/config_PRUEBA_IO.json"
